...
func StringIsIntInRange(min int, max int) schema.SchemaValidateFunc {
	return func(i interface{}, k string) (warnings []string, errors []error) {
		...
		if err != nil {
			errors = append(errors, fmt.Errorf("expected %s to string representation of integer", k))
			return warnings, errors
		}
		if intVal < min || intVal > max {
			errors = append(errors, fmt.Errorf("expected %s to be between %d and %d inclusive", k, min, max))
			return warnings, errors
		}
		return warnings, errors
	}
